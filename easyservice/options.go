...
func (that *EasyService) parserConfig(parser *gcmd.Parser) {
	...
	} else if len(that.config.GetAdapter().(*gcfg.AdapterFile).MustGet(context.TODO(), "ENV_NAME").String()) <= 0 {
	...
	if index != -1 {
		...
		debug := parser.GetOpt("debug", that.config.MustGet(context.TODO(), "Debug", genv.Get("DEBUG", false).Bool()))
