...
func (that *EasyService) initLogSetting(config *gcfg.Config) error {
	loggerCfg := gjson.New(config.MustGet(context.TODO(), "logger"))
	env := loggerCfg.Get("ENV_NAME").String()
	level := loggerCfg.Get("Level").String()
	...
	logPath := loggerCfg.Get("Path").String()
	if len(logPath) > 0 {
