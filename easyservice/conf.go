...
func NewBoxConf(name string, config *gcfg.Config, parsers ...*gcmd.Parser) *BoxConf {
	cfg := &BoxConf{}
	ctx := context.TODO()
	cfg.id = config.MustGet(ctx, fmt.Sprintf("%s.Id", name), 0).Int()
	cfg.SandBoxName = config.MustGet(ctx, fmt.Sprintf("%s.Name", name), "").String()

	cfg.Network = config.MustGet(ctx, fmt.Sprintf("%s.Network", name), "tcp").String()
	host := config.MustGet(ctx, fmt.Sprintf("%s.Host", name), "0.0.0.0").String()
	port := config.MustGet(ctx, fmt.Sprintf("%s.Port", name), 0).Int()
	cfg.ListenAddress = fmt.Sprintf("%s:%d", host, port)

	cfg.SessionMaxTimeout = config.MustGet(ctx, fmt.Sprintf("%s.SessionMaxTimeout", name), 0).Duration()
	cfg.ResponseMaxTimeout = config.MustGet(ctx, fmt.Sprintf("%s.ResponseMaxTimeout", name), 0).Duration()
	cfg.RequestMaxTimeout = config.MustGet(ctx, fmt.Sprintf("%s.RequestMaxTimeout", name), 0).Duration()
	cfg.SlowTimeout = config.MustGet(ctx, fmt.Sprintf("%s.SlowTimeout", name), 0).Duration()
	cfg.CountTime = config.MustGet(ctx, fmt.Sprintf("%s.CountTime", name), false).Bool()
	cfg.RedialTimes = config.MustGet(ctx, fmt.Sprintf("%s.RedialTimes", name), 0).Int()
	cfg.RedialInterval = config.MustGet(ctx, fmt.Sprintf("%s.RedialTimes", name), 0).Duration()

	debug := config.MustGet(ctx, "Debug", false).Bool()
