	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(BeforeNewEndpointPlugin); ok {
			if err = _plugin.BeforeNewEndpoint(endpointConfig, that); err != nil {
				logger.Fatalf(context.TODO(), "[BeforeNewEndpoint:%s] %s", plugin.Name(), err.Error())
				return
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterNewEndpointPlugin); ok {
			if err = _plugin.AfterNewEndpoint(e); err != nil {
				logger.Fatalf(context.TODO(), "[AfterNewEndpoint:%s] %s", plugin.Name(), err.Error())
				return
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(BeforeCloseEndpointPlugin); ok {
			if err = _plugin.BeforeCloseEndpoint(endpoint); err != nil {
				logger.Fatalf(context.TODO(), "[BeforeCloseEndpoint:%s] %s", plugin.Name(), err.Error())
				return err
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterRegRouterPlugin); ok {
			if err = _plugin.AfterRegRouter(h); err != nil {
				logger.Fatalf(context.TODO(), "[AfterRegRouter:%s] register handler:%s %s, error:%s", plugin.Name(), h.RouterTypeName(), h.Name(), err.Error())
				return
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterListenPlugin); ok {
			if err = _plugin.AfterListen(addr); err != nil {
				logger.Fatalf(context.TODO(), "[AfterListenPlugin:%s] network:%s, addr:%s, error:%s", plugin.Name(), addr.Network(), addr.String(), err.Error())
				return
			}
		}
	}
...
func (that *pluginSingleContainer) beforeDial(addr string, isRedial bool) (stat *Status) {
	...
	defer func() {
		if p := recover(); p != nil {
			logger.Errorf(context.TODO(), "[BeforeDialPlugin:%s]  panic:%v\n%s", pluginName, p, status.PanicStackTrace())
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(BeforeDialPlugin); ok {
			...
			if stat = _plugin.BeforeDial(addr, isRedial); !stat.OK() {
				logger.Debugf(context.TODO(), "[BeforeDialPlugin:%s] is_redial:%v, error:%s",
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterDialPlugin); ok {
			...
			if stat = _plugin.AfterDial(sess, isRedial); !stat.OK() {
				logger.Debugf(context.TODO(), "[AfterDialPlugin:%s] network:%s, addr:%s, is_redial:%v, error:%s",
...
func (that *pluginSingleContainer) afterDialFail(sess EarlySession, err error, isRedial bool) (stat *Status) {
	...
	defer func() {
		if p := recover(); p != nil {
			logger.Errorf(context.TODO(), "[AfterDialFailPlugin:%s] network:%s, addr:%s, panic:%v\n%s", pluginName, sess.RemoteAddr().Network(), sess.RemoteAddr().String(), p, status.PanicStackTrace())
			stat = statDialFailed.Copy(p)
		}
	}()
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterDialFailPlugin); ok {
			...
			if stat = _plugin.AfterDialFail(sess, err, isRedial); !stat.OK() {
				logger.Debugf(context.TODO(), "[AfterDialFailPlugin:%s] network:%s, addr:%s, is_redial:%v, error:%s",
					pluginName, sess.RemoteAddr().Network(), sess.RemoteAddr().String(), isRedial, stat.String(),
				)
				return stat
			}
...
func (that *pluginSingleContainer) afterAccept(sess EarlySession) (stat *Status) {
	...
	defer func() {
		if p := recover(); p != nil {
			logger.Errorf(context.TODO(), "[AfterAcceptPlugin:%s] network:%s, addr:%s, panic:%v\n%s", pluginName, sess.RemoteAddr().Network(), sess.RemoteAddr().String(), p, status.PanicStackTrace())
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterAcceptPlugin); ok {
			...
			if stat = _plugin.AfterAccept(sess); !stat.OK() {
				logger.Debugf(context.TODO(), "[PostAcceptPlugin:%s] network:%s, addr:%s, error:%s", pluginName, sess.RemoteAddr().Network(), sess.RemoteAddr().String(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(BeforeWriteCallPlugin); ok {
			if stat = _plugin.BeforeWriteCall(ctx); !stat.OK() {
				logger.Debugf(context.TODO(), "[BeforeWriteCallPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterWriteCallPlugin); ok {
			if stat = _plugin.AfterWriteCall(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[AfterWriteCallPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(BeforeWriteReplyPlugin); ok {
			if stat = _plugin.BeforeWriteReply(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[BeforeWriteReplyPlugin:%s] %s", plugin.Name(), stat.String())
				return
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterWriteReplyPlugin); ok {
			if stat = _plugin.AfterWriteReply(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[AfterWriteReplyPlugin:%s] %s", plugin.Name(), stat.String())
				return
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(BeforeWritePushPlugin); ok {
			if stat = _plugin.BeforeWritePush(ctx); !stat.OK() {
				logger.Debugf(context.TODO(), "[BeforeWritePushPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterWritePushPlugin); ok {
			if stat = _plugin.AfterWritePush(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[AfterWritePushPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(BeforeReadHeaderPlugin); ok {
			if err = _plugin.BeforeReadHeader(ctx); err != nil {
				logger.Debugf(context.TODO(), "[BeforeReadHeaderPlugin:%s] disconnected when reading: %s", plugin.Name(), err.Error())
				return err
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterReadCallHeaderPlugin); ok {
			if stat = _plugin.AfterReadCallHeader(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[AfterReadCallHeaderPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(BeforeReadCallBodyPlugin); ok {
			if stat = _plugin.BeforeReadCallBody(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[BeforeReadCallBodyPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterReadCallBodyPlugin); ok {
			if stat = _plugin.AfterReadCallBody(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[AfterReadCallBodyPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterReadPushHeaderPlugin); ok {
			if stat = _plugin.AfterReadPushHeader(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[AfterReadPushHeaderPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(BeforeReadPushBodyPlugin); ok {
			if stat = _plugin.BeforeReadPushBody(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[BeforeReadPushBodyPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterReadPushBodyPlugin); ok {
			if stat = _plugin.AfterReadPushBody(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[AfterReadPushBodyPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterReadReplyHeaderPlugin); ok {
			if stat = _plugin.AfterReadReplyHeader(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[AfterReadReplyHeaderPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(BeforeReadReplyBodyPlugin); ok {
			if stat = _plugin.BeforeReadReplyBody(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[BeforeReadReplyBodyPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterReadReplyBodyPlugin); ok {
			if stat = _plugin.AfterReadReplyBody(ctx); !stat.OK() {
				logger.Errorf(context.TODO(), "[AfterReadReplyBodyPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
	...
	for _, plugin := range that.plugins {
		if _plugin, ok := plugin.(AfterDisconnectPlugin); ok {
			if stat = _plugin.AfterDisconnect(sess); !stat.OK() {
				logger.Errorf(context.TODO(), "[AfterDisconnectPlugin:%s] %s", plugin.Name(), stat.String())
				return stat
			}
		}
	}
