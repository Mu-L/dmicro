...
func (that *session) SetID(newID string) {
	...
	hub.set(that)
	hub.delete(oldID)
	logger.Infof(context.TODO(), "session changes id: %s -> %s", oldID, newID)
}
...
func (that *session) doSend(output message.Message) *status.Status {
	...
	default:
		...
		logger.Debugf(context.TODO(), "write error: %s", err.Error())
...
func (that *session) AsyncCall(serviceMethod string, args interface{}, result interface{}, callCmdChan chan<- CallCmd, setting ...message.MsgSetting) CallCmd {
	if callCmdChan == nil {
		...
		if cap(callCmdChan) == 0 {
			logger.Panicf(context.TODO(), "*session.AsyncCall(): callCmdChan channel is unbuffered")
		}
	}
...
func (that *session) startReadAndHandle(c context.Context) {
	...
	for that.goonRead() {
		...
		if err = grpool.Add(context.TODO(), func(c context.Context) {
...
func (that *session) readDisconnected(oldConn net.Conn, err error) {
	...
	if err != nil && err != socket.ErrProactivelyCloseSocket {
		...
		if errStr := err.Error(); errStr != "EOF" {
			reason = errStr
			//记录会话关闭原因
			logger.Warningf(context.TODO(), "disconnect when reading: %T %s", err, errStr)
		}
	}
	//优化的等待所有处理程序结束
	that.graceCtxWait()
